...
const PACKWIZ_URL: &str =
    "https://nightly.link/packwiz/packwiz/workflows/go/main/Windows%2064-bit.zip";
...
async fn run_install(
    ...
    let work_dir = work_dir.map(PathBuf::from).unwrap_or_else(default_work_dir);
...
fn maybe_download(url: &str, target: &Path, logs: &mut Vec<String>) -> anyhow::Result<()> {
    if is_valid_zip(target) {
        logs.push(format!("已存在且有效，跳过下载: {}", target.display()));
        return Ok(());
    }
    if target.exists() {
        logs.push(format!("检测到无效缓存，重新下载: {}", target.display()));
    }

    logs.push(format!("下载中: {url}"));
    let mut response = reqwest::blocking::get(url).context("下载请求失败")?;
    ...
    let download_result = (|| -> anyhow::Result<()> {
        let mut temp_file = File::create(&temp_target).context("创建临时下载文件失败")?;
        io::copy(&mut response, &mut temp_file).context("写入临时下载文件失败")?;
        temp_file.sync_all().ok();

        if target.exists() {
            fs::remove_file(target).context("清理旧下载文件失败")?;
        }
        fs::rename(&temp_target, target).context("替换下载文件失败")?;
        Ok(())
    })();
...
fn unpack_zip(zip_path: &Path, extract_to: &Path, logs: &mut Vec<String>) -> anyhow::Result<()> {
    ...
    for i in 0..archive.len() {
        ...
        io::copy(&mut item, &mut out_file).context("写入解压文件失败")?;
    }
